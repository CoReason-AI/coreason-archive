...
async def add_thought(req: ThoughtRequest, archive: CoreasonArchive = Depends(get_archive)):
    try:
        # Validate scope
        try:
            mem_scope = MemoryScope(req.scope)
        except ValueError:
            raise HTTPException(status_code=400, detail=f"Invalid scope: {req.scope}. Valid scopes: {[s.value for s in MemoryScope]}")
...
@app.post("/search")
async def search(req: SearchRequest, archive: CoreasonArchive = Depends(get_archive)):
    try:
...
@app.get("/health")
async def health(archive: CoreasonArchive = Depends(get_archive)):
